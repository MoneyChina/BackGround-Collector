...
exports.getWxUser = (req, res) => {
    ...
    request(urlStr, (error, response, body)=>{
        if (!error && response.statusCode == 200) {
            ...
            .then(result => { //数据库中有内容，拿用户信息
                //console.log(result)
                if (result) {
                    console.log('在数据库中已匹配到用户信息！[openid]已获取')                    
                    res.json({
                        code: 200,
                        message: '获取成功',
                        data: {
...
exports.TimeMachine = (req,res) => { // **需测试
    ...
    if(typ == 1){
        console.log('用户选择查看收藏历史')
        //console.log(typeof(time))
        User.find({openid})
        .then(result => {
            //console.log(result[0].saveusernews);
            //console.log(time == '1');
            if(time == '1'){
                var TimeArray = [];
                var newsArray = [];
